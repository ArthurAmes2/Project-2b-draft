...
Node* performGameSearchGraph(const string &query,  map<string, Node*> &vertexMap, Graph &graph) {
    auto graphClock = Clock::now(); //Starting the time taken to search in a Graph

    cout << "Searching for: " << query << " in Graph..." << endl;
    Node* result = graph.gameSearch(query, vertexMap);
    if (!result) {
        cout << "No match found for: " << query << endl;
        return nullptr;
    }

    cout << "MetaCritic Score: " << result->metacritic << endl;
    cout << "Released: " << result->released << endl;
    cout << "Updated: " << result->updated << endl;
    cout << "Website: " << result->website << endl;
    cout << "Rating: " << result->rating << endl;
    cout << "Platform(s):" << endl;
    for (auto platform: result->platforms) {
        cout << "   " << platform << endl;
    }
    cout << "Developer(s):" << endl;
    for (auto develepor: result->developers) {
        cout << "   " << develepor << endl;
    }
    cout << "Genre(s):" << endl;
    for (auto genre: result->genres) {
        cout << "   " << genre << endl;
    }
    cout << "Publisher(s):" << endl;
    for (auto publisher: result->publishers) {
        cout << "   " << publisher << endl;
    }
    cout << "ESRB Rating: " << result->esrb_rating << endl;


    cout << "Searching for similar games..." << endl;

    vector<pair<string, float>> similar_games = graph.similaritySearch(query, vertexMap);
    cout << endl;
    cout << "Similar games (name, score): " << endl;
    for (int i = 0; i < 21; i++) {
        cout << "   " << similar_games[i].first << " , " << similar_games[i].second << endl;
    }

    auto graphTime = chrono::duration_cast<chrono::milliseconds>(Clock::now() - graphClock).count();
    cout << "Graph Time Taken to Search: " << graphTime << " ms" << endl;

    return result;
}

Node* performGameSearchBPlus(const string &query,  map<string, Node*> &vertexMap, BPlusTree &tree) {
    auto bplusClock = Clock::now(); //Starting the time taken to search in a B Plus Tree
    cout << "Searching for: " << query << " in B+ Tree..." << endl;
    Node* result = tree.gameSearch(query, vertexMap);
    if (!result) {
        cout << "No match found for: " << query << endl;
        return nullptr;
    }
    ...
    vector<pair<string, float>> similar_games = tree.similaritySearch(query, vertexMap);
    cout << endl;
    cout << "Similar games (name, score): " << endl;
    for (int i = 0; i < 21; i++) {
        cout << "   " << similar_games[i].first << " , " << similar_games[i].second << endl;
    ...
    auto bplusTime = chrono::duration_cast<chrono::milliseconds>(Clock::now() - bplusClock).count();
    cout << "B+ Tree Time Taken to Search: " << bplusTime << " ms" << endl;
    return result;
}
...
int main() {
    ...
    // 0 = none, 1 = search_bar
    int focused = 0;
    //referenced from programming fundamentals 2 coding projects
    while (window.isOpen()) {
        sf::Event event;
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
                ...
                if (search_button.getGlobalBounds().contains(mp)) {
                    found = performGameSearchGraph(input_string, data_map, gameGraph);
                    performGameSearchBPlus(input_string, data_map, gameTree); 
                    if (found == nullptr) {
                        cout << "Game not found... Retry Again" << endl;
                        break;
                    }
